...
# Karta rasmi file_id: admin /card_setup buyrug'ini ishlatib rasm yuborsin
CARD_PHOTO_ID = ""   # Misol: "AgACAgIAAxkBAAI..."
DB_PATH      = "bot.db"
...
class AS(StatesGroup):
    ...
    ch_id        = State()
    ch_name      = State()
    # Start xabar ketma-ket: rasm -> matn -> qo'shimcha
    sm_photo     = State()
    sm_text      = State()
    sm_extra     = State()
    broadcast    = State()
...
# ═══ O'ZGARTIRISH 1: Instagram obunani tekshirmaslik ═══
async def unsub_channels(bot: Bot, uid: int) -> list:
    """Barcha kanallarni ko'rsatadi, lekin faqat Telegram obunasini tekshiradi."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
    ...
    for ch in chs:
        if ch['ch_type'] == 'instagram':
            # Instagram: har doim ko'rsatiladi (obuna tekshirilmaydi)
            result.append(ch)
        else:
            # Telegram: obunasini tekshiramiz
            try:
...
async def has_tg_unsub(bot: Bot, uid: int) -> bool:
    """Faqat Telegram kanallar tekshiriladi - True = obuna bo'lmagan bor."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
...
def kb_main(lg: str, adm: bool = False):
    rows = [
        [KeyboardButton(text=tr(lg, 'btn_videos'))],
...
def kb_admin():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Karta rasmi",        callback_data="adm:card_photo")],
...
async def cmd_start(msg: Message, bot: Bot, state: FSMContext):
    ...
    # Faqat Telegram kanallar uchun majburiy obuna tekshiruvi
    tg_unsub = await has_tg_unsub(bot, msg.from_user.id)
    all_chs  = await unsub_channels(bot, msg.from_user.id)
    if tg_unsub:
        await msg.answer(tr(lang(u), 'not_subbed'), reply_markup=kb_sub(all_chs))
        return
    elif all_chs:
        # Faqat Instagram kanallar bor — ko'rsatamiz, lekin bloklamaymiz
        await msg.answer(tr(lang(u), 'not_subbed'), reply_markup=kb_sub(all_chs))
...
async def cb_lang(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    tg_unsub = await has_tg_unsub(bot, call.from_user.id)
    all_chs  = await unsub_channels(bot, call.from_user.id)
    if tg_unsub:
        await call.message.answer(tr(lg, 'not_subbed'), reply_markup=kb_sub(all_chs))
        await call.answer()
        return
    elif all_chs:
        await call.message.answer(tr(lg, 'not_subbed'), reply_markup=kb_sub(all_chs))
...
async def cb_check_sub(call: CallbackQuery, bot: Bot):
    ...
    tg_unsub = await has_tg_unsub(bot, call.from_user.id)
    if tg_unsub:
        await call.answer("Hali obuna bo'lmagan Telegram kanallar bor!", show_alert=True)
        return
    lg  = lang(u)
...
# ─────────── Videolar ───────────
MENU_BUTTONS = {"Videolar", "Видео", "Hisobim", "Мой счёт", "Yordam", "Помощь", "Admin panel", "Админ панель"}
...
# ═══ O'ZGARTIRISH 2: Karta to'lov — rasm + nusxalash tugmasi ═══
async def send_card_message(target, amount: float):
    """Karta ma'lumotini rasm (agar sozlangan bo'lsa) yoki matn sifatida yuboradi."""
    text = (
    ...
    if CARD_PHOTO_ID:
        await target.answer_photo(CARD_PHOTO_ID, caption=text,
                                  parse_mode=ParseMode.HTML, reply_markup=copy_kb)
    else:
        await target.answer(text, parse_mode=ParseMode.HTML, reply_markup=copy_kb)

@router.callback_query(F.data == "copy_card")
async def cb_copy_card(call: CallbackQuery):
    await call.answer(f"Karta: {PAYMENT_CARD}", show_alert=True)
...
async def cb_paycard(call: CallbackQuery, state: FSMContext):
    ...
    await send_card_message(call.message, video['price'])
    await call.answer()
...
async def rx_topup_amount(msg: Message, state: FSMContext):
    ...
    # ═══ Karta rasmi bilan yuborish ═══
    await send_card_message(msg, amount)
...
async def vp_done(call: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"descs": [], "desc_idx": 0})
...
async def rx_part_desc(msg: Message, state: FSMContext):
    ...
    descs.append("" if text == "-" else text)
    parts = data.get("parts", [])
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        try:
            ...
            for i, p in enumerate(parts):
                ...
                await db.execute(
                    "INSERT INTO video_parts (video_id,part_number,file_id,description)"
                    " VALUES (?,?,?,?)",
                    (vid, p['num'], p['fid'], desc)
                )
            await db.commit()
            await msg.answer(
...
# ═══ O'ZGARTIRISH 3: Start xabar — ketma-ket forma (tugmasiz) ═══
# Jarayon: rasm (ixtiyoriy) → matn → qo'shimcha (iqtibos/link/text_link, ixtiyoriy)
...
async def adm_start_msg(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        "🖼 <b>1-qadam: Rasm yuboring</b>\n\n"
        "Rasm kerak bo'lmasa <code>-</code> yozing.",
        parse_mode=ParseMode.HTML
    )
    await call.answer()
...
async def rx_sm_photo(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "✏️ <b>2-qadam: Matn kiriting</b>\n\n"
        "HTML ishlatsa bo'ladi: <code>&lt;b&gt;qalin&lt;/b&gt;</code>, "
...
async def rx_sm_text(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        "📎 <b>3-qadam: Qo'shimcha (ixtiyoriy)</b>\n\n"
        "Kerak bo'lmasa <code>-</code> yozing.\n\n"
        "Yoki quyidagilardan birini kiriting:\n"
        "• <b>Iqtibos muallifi</b>: <code>Alisher Navoiy</code>\n"
        "• <b>Havola URL</b>: <code>https://t.me/kanal</code>\n"
        "• <b>Matnli havola</b>: <code>https://t.me/kanal|Kanalimiz</code>",
        parse_mode=ParseMode.HTML
    )
...
async def rx_sm_extra(msg: Message, state: FSMContext):
    ...
    if text and text != "-":
        ...
        if text.startswith("http") and "|" in text:
            mtype = "photo" if photo else "text_link"
        elif text.startswith("http"):
            mtype = "photo" if photo else "link"
        else:
            mtype = "photo" if photo else "quote"
    else:
        mtype = "photo" if photo else "text"

    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("DELETE FROM start_msg")
        await db.execute(
            "INSERT INTO start_msg (id,msg_type,content,photo_id,extra) VALUES (1,?,?,?,?)",
            (mtype, content, photo, extra)
        )
        await db.commit()
    ...
    await msg.answer(
        f"✅ <b>Start xabar saqlandi!</b>\n\nTur: <b>{type_names.get(mtype, mtype)}</b>",
        parse_mode=ParseMode.HTML
    )

# ─────────── Karta rasmi sozlash (admin) ───────────
_waiting_card_photo = set()

@router.callback_query(F.data == "adm:card_photo")
async def adm_card_photo(call: CallbackQuery):
    if not await is_admin(call.from_user.id):
        return
    _waiting_card_photo.add(call.from_user.id)
    current = f"<code>{CARD_PHOTO_ID}</code>" if CARD_PHOTO_ID else "Sozlanmagan"
    await call.message.answer(
        f"💳 <b>Karta rasmi</b>\n\n"
        f"Hozirgi holat: {current}\n\n"
        f"Yangi karta rasmini yuboring.\n"
        f"(Keyin uni <code>CARD_PHOTO_ID</code> ga qo'yishingiz kerak)",
        parse_mode=ParseMode.HTML
    )
    await call.answer()

@router.message(F.photo)
async def catch_any_photo(msg: Message):
    if msg.from_user.id in _waiting_card_photo and await is_admin(msg.from_user.id):
        _waiting_card_photo.discard(msg.from_user.id)
        fid = msg.photo[-1].file_id
        await msg.answer(
            f"✅ <b>Rasm qabul qilindi!</b>\n\n"
            f"File ID:\n<code>{fid}</code>\n\n"
            f"Bot.py faylingizda:\n"
            f"<code>CARD_PHOTO_ID = \"{fid}\"</code>\n\n"
            f"Shu qatorni o'zgartirib botni qayta ishga tushiring.",
            parse_mode=ParseMode.HTML
        )
