...
# ═══════════════════════ MIDDLEWARE ═══════════════════════
# Bu middleware: foydalanuvchi ISTALGAN state da menu tugmasini bossa —
# state ni tozalab, so'rovni oddiy handler ga o'tkazadi.
    ...
    async def __call__(
        ...
        if isinstance(event, Message) and event.text in MENU_BUTTONS:
            ...
            if state:
                ...
                # Faqat user state larini tozala (admin state larini emas)
                # Chunki admin state lari ham menu tugmalaridan interrupt bo'lishi kerak
...
T = {
    'uz': {
        ...
        'already_bought' : "Siz bu videoni oldin sotib olgansiz. Qismni tanlang:",
...
async def make_stats_image() -> bytes:
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute("""
            SELECT
                u.telegram_id, u.full_name, u.balance,
            ...
            ORDER BY u.joined_at DESC LIMIT 40
        """) as c:
    ...
    for r in rows:
        ...
        table_data.append([
            name, str(r['telegram_id']),
            f"{r['balance']:,.0f}", f"{r['spent']:,.0f}",
            r['last_code'] or "-", str(r['shares']), fmt_dt(r['joined_at']),
        ])
    if not table_data:
        table_data = [["-"] * 7]
...
async def cmd_start(msg: Message, bot: Bot, state: FSMContext):
    ...
    if not_sub:
        if await unsub_telegram_only(bot, msg.from_user.id):
            await msg.answer(tr(lang(u), 'not_subbed'), reply_markup=kb_sub(not_sub))
            return
        await msg.answer(tr(lang(u), 'not_subbed'), reply_markup=kb_sub(not_sub))

    await send_start(msg, bot, u, uid=msg.from_user.id)

async def send_start(msg: Message, bot: Bot, u, uid: int = None):
    lg  = lang(u)
    adm = await is_admin(uid or msg.from_user.id)
    ...
    if sm:
        ...
        if mtype == 'photo' and photo:
            await msg.answer_photo(photo, caption=content, parse_mode=ParseMode.HTML, reply_markup=kb)
        elif mtype == 'quote':
...
async def cb_lang(call: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if not_sub:
        if await unsub_telegram_only(bot, call.from_user.id):
            await call.message.answer(tr(lg, 'not_subbed'), reply_markup=kb_sub(not_sub))
            await call.answer()
            return
        await call.message.answer(tr(lg, 'not_subbed'), reply_markup=kb_sub(not_sub))
    await send_start(call.message, bot, u, uid=call.from_user.id)
    await call.answer()
...
async def btn_videos(msg: Message, state: FSMContext):
    # state middleware tomonidan allaqachon tozalangan
    u = await get_user(msg.from_user.id)
    await state.set_state(US.code)
...
async def cb_paycard(call: CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute("SELECT price FROM videos WHERE id=?", (vid,)) as c:
            video = await c.fetchone()
    await state.clear()
    await state.update_data({"vid": vid, "amount": video['price']})
...
async def btn_account(msg: Message, state: FSMContext):
    # state middleware tomonidan allaqachon tozalangan
    u  = await get_user(msg.from_user.id)
    lg = lang(u)
    txt = tr(lg, 'bal_info',
...
async def rx_help(msg: Message, state: FSMContext, bot: Bot):
    ...
    for aid in ADMIN_IDS:
        try:
            if mtype == 'photo':
                await bot.send_photo(aid, fid, caption=adm_txt, parse_mode=ParseMode.HTML, reply_markup=rkb)
            elif mtype == 'document':
                await bot.send_document(aid, fid, caption=adm_txt, parse_mode=ParseMode.HTML, reply_markup=rkb)
            elif mtype == 'voice':
                await bot.send_voice(aid, fid, caption=adm_txt, parse_mode=ParseMode.HTML, reply_markup=rkb)
            elif mtype == 'video':
                await bot.send_video(aid, fid, caption=adm_txt, parse_mode=ParseMode.HTML, reply_markup=rkb)
            elif mtype == 'sticker':
...
async def cb_replyhelp(call: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"reply_uid": int(uid)})
...
async def btn_admin(msg: Message, state: FSMContext):
    # state middleware tomonidan allaqachon tozalangan
...
async def ch_add_ig(call: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AS.ch_id)
    await call.message.answer("Instagram sahifa havolasini kiriting:\nMisol: https://instagram.com/mypage")
    await call.answer()
...
async def rx_ch_name(msg: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        try:
            await db.execute(
                "INSERT OR REPLACE INTO channels (ch_id,ch_name,ch_type,added_at) VALUES (?,?,?,?)",
                (data['ch_id'], msg.text.strip(), data.get('ch_type', 'telegram'), now_tz().isoformat())
            )
            await db.commit()
            tp = "Telegram" if data.get('ch_type') == 'telegram' else "Instagram"
            await msg.answer(f"{tp} kanal qo'shildi: <b>{msg.text.strip()}</b>", parse_mode=ParseMode.HTML)
        except Exception as e:
            await msg.answer(f"Xato: {e}")
...
async def ch_list(call: CallbackQuery):
    ...
    for ch in chs:
        ...
        del_kb.append([InlineKeyboardButton(text=f"O'chirish: {ch['ch_name']}", callback_data=f"ch:del:{ch['id']}")])
    await call.message.answer("<b>Kanallar:</b>\n\n" + "\n".join(rows),
                              parse_mode=ParseMode.HTML,
...
async def rx_part_desc(msg: Message, state: FSMContext):
    ...
    await msg.answer(f"<b>{len(descs)}-qism</b> narxini kiriting (bepul = 0):", parse_mode=ParseMode.HTML)
...
async def rx_part_price(msg: Message, state: FSMContext):
    ...
    if len(descs) < len(parts):
        await state.set_state(AS.part_desc)
        await msg.answer(f"<b>{len(descs)+1}-qism</b> uchun tavsif kiriting (yoki - yozing):", parse_mode=ParseMode.HTML)
    else:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        try:
            cur = await db.execute(
                ...
                (data['title'], data['code'], price, len(parts), now_tz().isoformat(), msg.from_user.id)
            )
            vid = cur.lastrowid
            for i, p in enumerate(parts):
                ...
                await db.execute(
                    "INSERT INTO video_parts (video_id,part_number,file_id,description,price) VALUES (?,?,?,?,?)",
            ...
            for i in range(len(parts)):
                ...
                p_label = "bepul" if p_price == 0 else f"{p_price:,.0f} so'm"
                parts_info += f"\n  {i+1}-qism: {p_label}"
            await msg.answer(
...
async def adm_subs(call: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT telegram_id,username,full_name,balance,total_spent,joined_at"
            " FROM users ORDER BY joined_at DESC LIMIT 50"
        ) as c:
            users = await c.fetchall()
        async with db.execute("SELECT COUNT(*) FROM users") as c:
            total = (await c.fetchone())[0]
    lines = [f"<b>Obunchilar ({total} ta):</b>\n"]
    for u in users:
        nm  = u['full_name'] or "Noma'lum"
        un  = f"@{u['username']}" if u['username'] else "-"
        lines.append(
            f"👤 {nm} | {un}\n"
            f"   ID: <code>{u['telegram_id']}</code> | Balans: {u['balance']:,.0f}\n"
            f"   Sana: {fmt_dt(u['joined_at'])}"
        )
    txt = "\n\n".join(lines)
    if len(txt) > 4000:
        txt = txt[:4000] + "\n\n..."
...
async def adm_broadcast(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Xabar yuboring (matn, rasm, video):")
    await call.answer()
...
async def rx_broadcast(msg: Message, state: FSMContext, bot: Bot):
    ...
    await msg.answer(f"Xabar yuborildi: {ok} ta\nYuborilmadi: {fail} ta")
...
async def adm_start_msg(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        ...
        "• <b>Matnli havola</b> — matn + ko'rinadigan nom",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def _save_start_msg(msg: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
            ...
            (data.get("mtype","text"), data.get("content",""), data.get("photo_id"), data.get("extra"))
        )
        await db.commit()
    mtype_names = {
...
async def main():
    ...
    # Middleware ni ro'yxatdan o'tkaz — BARCHA message handlerlardan OLDIN ishlaydi
    router.message.middleware(MenuInterruptMiddleware())
