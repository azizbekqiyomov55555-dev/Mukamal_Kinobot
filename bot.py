...
# Karta rasmi: admin botga karta rasmini yuborib, file_id ni shu yerga yozing.
# Bo'sh qolsa matnli xabar yuboriladi.
...
# ─────────── Karta to'lov ───────────
async def send_card_info(target, lg: str, amount: float):
    """Karta ma'lumotini rasm yoki matn sifatida yuboradi + nusxalash tugmasi"""
    caption = (
    ...
    if CARD_PHOTO_ID:
        ...
        await target.answer(caption, parse_mode=ParseMode.HTML, reply_markup=copy_kb)
...
async def cb_paycard(call: CallbackQuery, state: FSMContext):
    ...
    await send_card_info(call.message, lg, video['price'])
    await call.answer()
...
async def rx_topup_amount(msg: Message, state: FSMContext):
    ...
    await send_card_info(msg, lg, amount)
...
# ─────────── Start xabar (ketma-ket forma, tugmasiz) ───────────
...
async def adm_start_msg(call: CallbackQuery, state: FSMContext):
    ...
    await state.update_data({"mtype": "text"})
    await call.message.answer(
        "🖼 <b>1-qadam: Rasm yuboring</b>\n\n"
        "Agar rasm kerak bo'lmasa — <code>-</code> yozing",
        parse_mode=ParseMode.HTML
    )
    await call.answer()
...
async def rx_sm_photo(msg: Message, state: FSMContext):
    if msg.photo:
        await state.update_data({"photo_id": msg.photo[-1].file_id, "mtype": "photo"})
    else:
        await state.update_data({"photo_id": None, "mtype": "text"})
    await state.set_state(AS.sm_text)
    await msg.answer(
        "✏️ <b>2-qadam: Matn kiriting</b>\n\n"
        "HTML teglar ishlatsa bo'ladi: <code>&lt;b&gt;bold&lt;/b&gt;</code>, "
...
async def rx_sm_text(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        "Quyidagilardan birini yozing yoki <code>-</code> yozing o'tkazib yuborish uchun:\n\n"
        "• <b>Iqtibos muallifi</b> — masalan: <code>Alisher Navoiy</code>\n"
        "• <b>Havola URL</b> — masalan: <code>https://t.me/kanal</code>\n"
        "• <b>Matnli havola</b> — masalan: <code>https://t.me/kanal|Kanalimiz</code>",
        parse_mode=ParseMode.HTML
    )
...
async def rx_sm_extra(msg: Message, state: FSMContext):
    ...
    if text == '-' or not text:
        ...
        if text.startswith("http") and '|' in text:
            await state.update_data({"extra": text, "mtype": "text_link"
                                     if not data.get("photo_id") else "photo"})
        elif text.startswith("http"):
            await state.update_data({"extra": text, "mtype": "link"
                                     if not data.get("photo_id") else "photo"})
        else:
            # iqtibos muallifi
            await state.update_data({"extra": text, "mtype": "quote"
                                     if not data.get("photo_id") else "photo"})
...
async def _save_start_msg(msg: Message, state: FSMContext):
    ...
    # Agar rasm bor bo'lsa mtype=photo, extra ni saqlaymiz
    if photo:
        mtype = "photo"
    ...
    preview = f"<b>✅ Start xabar saqlandi!</b>\n\nTur: <b>"
    if photo:
        preview += "Rasm + matn"
    elif mtype == "quote":
        preview += "Iqtibos"
        ...
        preview += "Matnli havola"
    else:
        preview += "Matn"
    preview += "</b>"
    if content:
        preview += f"\n\nMatn: {content[:100]}{'...' if len(content) > 100 else ''}"
    await msg.answer(preview, parse_mode=ParseMode.HTML)

# ─────────── Admin: karta rasmi file_id olish ───────────
@router.message(F.text == "/getfileid")
async def cmd_getfileid(msg: Message):
    if not await is_admin(msg.from_user.id):
        return
    await msg.answer(
        "📷 Karta rasmini shu yerga yuboring.\n"
        "Men sizga <b>file_id</b> ni beraman — uni <code>CARD_PHOTO_ID</code> ga yozing.",
        parse_mode=ParseMode.HTML
    )

@router.message(F.photo & F.func(lambda m: True))
async def catch_photo_fileid(msg: Message):
    if await is_admin(msg.from_user.id):
        fid = msg.photo[-1].file_id
        await msg.answer(
            f"📋 <b>File ID:</b>\n<code>{fid}</code>\n\n"
            f"Bu ID ni bot.py dagi <code>CARD_PHOTO_ID = \"{fid}\"</code> ga yozing.",
            parse_mode=ParseMode.HTML
        )
