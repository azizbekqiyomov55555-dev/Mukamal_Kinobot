...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    photo=get("start_photo"); text=get("start_text","🎬 <b>Kino botga xush kelibsiz!</b>\n\nKino kodini yuboring.")
    if photo: await update.message.reply_photo(photo=photo,caption=text,parse_mode=ParseMode.HTML,reply_markup=main_kb())
    else: await update.message.reply_text(text,parse_mode=ParseMode.HTML,reply_markup=main_kb())
...
async def cb_sub_check(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if nj:
        ...
        await q.message.delete(); u=q.from_user; photo=get("start_photo"); text=get("start_text","🎬 <b>Kino botga xush kelibsiz!</b>\n\nKino kodini yuboring.")
        if photo: await ctx.bot.send_photo(chat_id=u.id,photo=photo,caption=text,parse_mode=ParseMode.HTML,reply_markup=main_kb())
        else: await ctx.bot.send_message(chat_id=u.id,text=text,parse_mode=ParseMode.HTML,reply_markup=main_kb())
...
async def adm_stats(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    rows=c.execute("SELECT u.tg_id,u.full_name,u.username,u.balance,u.joined_at,COALESCE(SUM(CASE WHEN t.kind='topup' AND t.status='approved' THEN t.amount ELSE 0 END),0) FROM users u LEFT JOIN txs t ON u.tg_id=t.tg_id GROUP BY u.tg_id ORDER BY u.joined_at DESC LIMIT 20").fetchall()
    ...
    c.close()
    mv_txt="".join(f"\n  🎬 {m[0]} – {m[1]}-qism | {m[2]}×" for m in top_mv)
    u_txt="".join(f"\n👤 {u[1]}  🆔<code>{u[0]}</code>  @{u[2] or '–'}  💰{u[3]:,.0f}  📥{u[5]:,.0f}  📅{u[4]}" for u in rows)
    text=(f"📊 <b>Statistika</b>\n🕐 {now()}\n\n"
          f"👥 Jami foydalanuvchi: <b>{total}</b>\n"
          f"📅 Bugun: <b>{today}</b>  |  Hafta: <b>{week}</b>\n"
          f"🎬 Kinolar (kod): <b>{movies}</b>\n\n"
          f"💰 Jami kiritildi: <b>{t_top:,.0f} so'm</b>\n"
          f"🛒 Jami sotuvlar: <b>{t_sal:,.0f} so'm</b>\n\n"
          f"🔝 <b>Eng ko'p ko'rilganlar:</b>{mv_txt}\n\n"
          f"👥 <b>Foydalanuvchilar (so'nggi 20):</b>{u_txt}")
    await update.message.reply_text(text,parse_mode=ParseMode.HTML,reply_markup=adm_kb())
