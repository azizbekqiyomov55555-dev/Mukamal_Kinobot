...
async def btn_videos(msg: Message, state: FSMContext):
    ...
    await msg.answer(tr(lang(u), 'enter_code'), reply_markup=ReplyKeyboardRemove())
...
async def vp_done(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        f"{n} ta qism qabul qilindi.\n\n<b>1-qism</b> uchun tavsif kiriting:",
        parse_mode=ParseMode.HTML
    )
    await call.answer()
...
async def rx_part_desc(msg: Message, state: FSMContext):
    ...
    descs.append(msg.text.strip())
    parts = data.get("parts", [])
    if len(descs) < len(parts):
        await state.update_data({"descs": descs})
        await msg.answer(f"<b>{len(descs)+1}-qism</b> uchun tavsif kiriting:",
                         parse_mode=ParseMode.HTML)
    else:
        await state.update_data({"descs": descs})
...
async def adm_start_msg(call: CallbackQuery, state: FSMContext):
    if not await is_admin(call.from_user.id):
        return
    await state.set_state(AS.start_type)
    await call.message.answer(
        "Start xabar turini tanlang:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Oddiy matn",  callback_data="st:text")],
            [InlineKeyboardButton(text="Rasm + matn", callback_data="st:photo")],
            [InlineKeyboardButton(text="Iqtibos",     callback_data="st:quote")],
            [InlineKeyboardButton(text="Link",        callback_data="st:link")],
        ])
    )
    await call.answer()

@router.callback_query(F.data.startswith("st:"))
async def cb_st_type(call: CallbackQuery, state: FSMContext):
    mtype = call.data.split(":")[1]
    await state.update_data({"mtype": mtype})
    if mtype == 'photo':
        await state.set_state(AS.start_photo)
        await call.message.answer("Rasm yuboring:")
    else:
        await state.set_state(AS.start_text)
        await call.message.answer("Matn kiriting:")
    await call.answer()

@router.message(AS.start_photo, F.photo)
async def rx_start_photo(msg: Message, state: FSMContext):
    await state.update_data({"photo_id": msg.photo[-1].file_id})
    await state.set_state(AS.start_text)
    await msg.answer("Rasm uchun matn kiriting:")

@router.message(AS.start_text)
async def rx_start_text(msg: Message, state: FSMContext):
    data  = await state.get_data()
    await state.update_data({"content": msg.text or ""})
    mtype = data.get("mtype", "text")
    if mtype in ('quote', 'link'):
        await state.set_state(AS.start_extra)
        lbl = "Manba (muallif):" if mtype == 'quote' else "URL kiriting:"
        await msg.answer(lbl)
    else:
        await save_start_msg(msg, state)

@router.message(AS.start_extra)
async def rx_start_extra(msg: Message, state: FSMContext):
    await state.update_data({"extra": msg.text or ""})
    await save_start_msg(msg, state)

async def save_start_msg(msg: Message, state: FSMContext):
    data = await state.get_data()
    await state.clear()
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.commit()
    await msg.answer("Start xabar saqlandi!")
