...
def kb_admin():
    return ReplyKeyboardMarkup([
        ...
        ["📢 Kanal boshqaruv", "⚙️ Start xabari"],
        ["📨 Barchaga xabar",  "📩 ID'ga xabar"],
        ["💵 ID'ga pul",       "📊 Statistika"],
...
async def cb_sub_check(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if nj:
        try:
            ...
            await ctx.bot.send_message(uid, "⚠️ Hali obuna bo'lmagan kanallar bor:",
                                        reply_markup=sub_keyboard(nj))
    else:
        try:
    ...
    for aid in ADMIN_IDS:
        try:
            ...
            cap = (f"💰 <b>HISOBNI TO'LDIRISH</b>\n\n"
                   f"👤 {user.full_name}\n🆔 <code>{user.id}</code>\n"
                   f"📱 @{user.username or '–'}\n💵 <b>{amount:,.0f} so'm</b>\n🕐 {now_str()}")
            await ctx.bot.send_photo(chat_id=aid, photo=fid, caption=cap,
                                      reply_markup=kb_adm, parse_mode=ParseMode.HTML)
        except Exception as e:
            log.error(f"topup notify: {e}")
    await update.message.reply_text(
...
async def cb_approve_topup(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await q.edit_message_caption(caption=old + "\n\n✅ <b>TASDIQLANDI</b>",
                                      parse_mode=ParseMode.HTML)
    except: pass
...
async def show_parts(update: Update, ctx: ContextTypes.DEFAULT_TYPE, code: str):
    ...
    for p in parts:
        ...
        btns.append([InlineKeyboardButton(
            f"📽 {p['part_no']}-qism  —  {price_str}",
            callback_data=f"qism|{code}|{p['part_no']}")])
    await update.message.reply_text(
        f"🎬 <b>Kod: {code}</b>\n\nQismni tanlang:",
...
async def cb_qism(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if price == 0:
        ...
        await ctx.bot.send_video(chat_id=uid, video=part["file_id"],
    ...
    if bal >= price:
        ...
        kb.append([InlineKeyboardButton(
            "💳 Hisobni to'ldirish", callback_data="topup_open")])
    kb.append([InlineKeyboardButton(
        "💴 Karta orqali to'lash", callback_data=f"pay_card|{code}|{pno}")])
    try:
...
async def cb_pay_bal(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with db_conn() as c:
        c.execute(
            "INSERT INTO txs(tg_id,amount,kind,code,part_no,status,created_at) VALUES(?,?,?,?,?,?,?)",
...
async def mv_card_recv(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with db_conn() as c:
        c.execute(
            "INSERT INTO txs(tg_id,amount,kind,code,part_no,status,file_id,created_at) VALUES(?,?,?,?,?,?,?,?)",
...
async def support_recv(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for aid in ADMIN_IDS:
        try:
            ...
            await ctx.bot.send_message(aid, "Javob berish:",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("✍️ Javob yozish", callback_data=f"reply|{user.id}")
                ]]))
        except Exception as e:
            log.error(f"support_recv: {e}")
...
async def a_code(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📝 1-qism uchun ma'lumot kiriting:", reply_markup=kb_cancel())
    return A_INFO
...
async def a_price(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with db_conn() as c:
        for i, (vid, inf, pr) in enumerate(
                zip(ctx.user_data["vids"], ctx.user_data["infos"], ctx.user_data["prices"]), 1):
            c.execute("INSERT INTO parts(code,part_no,file_id,info,price) VALUES(?,?,?,?,?)",
                      (code, i, vid, inf, pr))
    await update.message.reply_text(
        f"✅ Kino saqlandi!\n🔑 Kod: <code>{code}</code>\n📽 Qismlar: {len(ctx.user_data['vids'])}",
...
async def cont_price(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ {pno}-qism saqlandi! Kod: <code>{code}</code>",
        parse_mode=ParseMode.HTML, reply_markup=kb_admin())
    return ConversationHandler.END
...
async def start_msg_open(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🖼 Rasm: {'✅ Bor' if cur_photo else '❌ Yo\'q'}\n\n"
        f"📝 Joriy matn:\n{cur_text}\n\n"
        f"━━━━━━━━━━━━━━━━━\n"
...
async def st_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        ...
        await update.message.reply_text("✅ Rasm qabul qilindi!\n\n"
            ...
            "(qanday kiritsangiz HUDDI SHUNDAY ko'rinadi):",
            reply_markup=kb_cancel())
    elif update.message.text == "⏭ O'tkazib yuborish":
        ctx.user_data["new_photo"] = cfg_get("start_photo", "")
        await update.message.reply_text(
            "📝 Start xabari matnini kiriting\n"
            "(qanday kiritsangiz HUDDI SHUNDAY ko'rinadi):",
            reply_markup=kb_cancel())
    else:
        await update.message.reply_text("❌ Rasm yuboring yoki «⏭ O'tkazib yuborish»ni bosing!")
        return ST_PHOTO
    return ST_TEXT
...
async def st_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ Start xabari saqlandi!\n\n"
        f"🖼 Rasm: {'✅ Bor' if new_photo else '❌ Yo\'q'}\n"
        f"📝 Matn:\n{new_text}",
        reply_markup=kb_admin())
    return ConversationHandler.END

# ══════════════════════════════════════════════════════════
#  ADMIN: KANAL BOSHQARUV
# ══════════════════════════════════════════════════════════
...
async def ch_action(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if t == "📋 Kanallar ro'yxati":
        chs = get_channels()
        if not chs:
            await update.message.reply_text("📢 Hali hech qanday kanal yo'q.", reply_markup=kb_admin())
        else:
            lst = "\n\n".join(
                f"[{c['id']}] <b>{c['name']}</b>\n   ID: <code>{c['cid']}</code>\n   Link: {c['link']}"
                for c in chs)
...
async def ch_del_id(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not t.isdigit():
        await update.message.reply_text("❌ Raqam kiriting!"); return CH_DEL_ID
    with db_conn() as c:
        r = c.execute("SELECT name FROM channels WHERE id=?", (int(t),)).fetchone()
        if not r:
            await update.message.reply_text("❌ Bunday raqamli kanal topilmadi!"); return CH_DEL_ID
...
async def ch_name_save(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with db_conn() as c:
        c.execute("INSERT INTO channels(cid,link,name) VALUES(?,?,?)",
                  (ctx.user_data["ch_cid"], ctx.user_data["ch_link"], name))
    await update.message.reply_text(
        f"✅ Kanal qo'shildi!\n📛 Nom: {name}\n🆔 ID: {ctx.user_data['ch_cid']}",
        reply_markup=kb_admin())
    return ConversationHandler.END
...
async def bc_send(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for i, u in enumerate(users):
        try:
            ...
            elif msg.photo:    await ctx.bot.send_photo(u["tg_id"], msg.photo[-1].file_id, caption=msg.caption or "")
            elif msg.video:    await ctx.bot.send_video(u["tg_id"], msg.video.file_id, caption=msg.caption or "")
...
async def snd_id(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not t.lstrip("-").isdigit():
        await update.message.reply_text("❌ To'g'ri ID kiriting!"); return SND_ID
...
async def bal_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    await update.message.reply_text(
        "💵 Pul qo'shiladigan foydalanuvchi ID sini kiriting:", reply_markup=kb_cancel())
    return BAL_ID
...
async def bal_id(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    t = update.message.text.strip()
    if not t.lstrip("-").isdigit():
        await update.message.reply_text("❌ To'g'ri ID kiriting!"); return BAL_ID
    uid = int(t); u = get_user(uid)
    if not u:
        await update.message.reply_text("❌ Bu foydalanuvchi topilmadi!"); return BAL_ID
...
async def bal_amt(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    t = update.message.text.strip().replace(" ", "").replace(",", "")
    if not t.lstrip("-").isdigit():
        await update.message.reply_text("❌ Faqat raqam kiriting!"); return BAL_AMT
    uid = ctx.user_data["bal_uid"]; amt = int(t)
    add_balance(uid, amt)
    with db_conn() as c:
        c.execute("INSERT INTO txs(tg_id,amount,kind,status,created_at) VALUES(?,?,?,?,?)",
                  (uid, amt, "topup", "approved", now_str()))
    try:
        await ctx.bot.send_message(uid,
            f"🎁 Hisobingizga <b>{amt:,.0f} so'm</b> qo'shildi!\n"
            f"💰 Balans: <b>{get_balance(uid):,.0f} so'm</b>", parse_mode=ParseMode.HTML)
    except: pass
    await update.message.reply_text(
        f"✅ {uid} ga {amt:,.0f} so'm qo'shildi!\nYangi balans: <b>{get_balance(uid):,.0f} so'm</b>",
        parse_mode=ParseMode.HTML, reply_markup=kb_admin())
    return ConversationHandler.END
...
async def adm_stats(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with db_conn() as c:
        ...
        top_u  = c.execute(
            "SELECT u.tg_id, u.full_name, u.balance FROM users u "
            "ORDER BY u.balance DESC LIMIT 5").fetchall()
    ...
    if PIL_OK:
        try:
            ...
            for y in range(H):
                r = 15 + int(y * 0.03)
                draw.line([(0,y),(W,y)], fill=(r, r, 15+int(y*0.05)))
            ...
            for i, u in enumerate(top_u):
                draw.text((16, 405 + i*20),
                    f"{i+1}. {u['full_name'][:22]}  —  {u['balance']:,.0f} so'm",
                    fill=(200,220,255))
...
async def cmd_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        ...
        return
    await update.message.reply_text("👨‍💼 <b>Admin paneli</b>",
                                     parse_mode=ParseMode.HTML, reply_markup=kb_admin())
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(uid):
        ...
        if t == "🏠 Asosiy menyu":
            await update.message.reply_text("Asosiy menyu:", reply_markup=kb_main()); return
...
def main():
    ...
    app.add_handler(cv(
        ...
        {MV_CARD_CHECK: [
            MessageHandler(filters.PHOTO | filters.Document.ALL, mv_card_recv),
            MessageHandler(filters.TEXT & ~filters.COMMAND, mv_card_recv)
        ]}))
